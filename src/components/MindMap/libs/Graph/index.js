...
export default class Graph {
    ...
    init(renderer) {
        ...
        this.root = this._initRoot(this.root);
    }
    ...
    addSubNode(item) {
        if(this.selected) {
            console.log('选中了', this.selected)
            return this._addNodeData(this.selected, item)
        }
    }
    _setSubParentData(parent, child) {
    ...
    _setNodeDirection(node) {
        ...
        // node.direction = node.direction || (father && father.direction ? father.direction : 1) 
    }
